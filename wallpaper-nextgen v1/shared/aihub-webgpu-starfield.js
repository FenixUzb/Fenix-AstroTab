...
class AIHubWebGPUStarfield {
  ...
  _buildInitialStars() {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const mag = Math.pow(Math.random(), 4); // few bright, many dim
      const intensity = 0.18 + mag * 0.82;
      const sizePx = 0.6 + Math.pow(intensity, 1.4) * 2.6;
      data[o + 4] = r;
      data[o + 5] = g;
      data[o + 6] = b;
      data[o + 7] = sizePx;
...
fn vs_main(@builtin(vertex_index) vid: u32, @builtin(instance_index) iid: u32) -> VSOut {
  ...
  // Scintillation: dim stars wobble hard, bright stars steady.
  let twinkleAmp = 0.05 + (1.0 - intensity) * (0.45 + u.audioHigh * 0.25);
  ...
  let sizePx = s.color.w * (1.0 + u.glowLift * intensity * 0.35) * audioScale * u.dpr;
  ...
  out.uv = corner;
  out.color = s.color.rgb * twinkle * (1.0 + u.audioHigh * 0.38 + u.audioBeat * intensity * 0.28);
...
fn fs_main(in: VSOut) -> @location(0) vec4<f32> {
  ...
  let halo = smoothstep(1.0, 0.25, d) * 0.18 * in.bright;
