...
function buildWarpStars() {
  const target = state.reducedMotion ? 240 : 520;
  if (state.warpStars.length === target) return;
  state.warpStars = [];
  for (let i = 0; i < target; i++) state.warpStars.push(makeWarpStar(true));
}
...
function drawStaticStars(ctx, dt, now) {
  ...
  if (!hideStars) {
    ...
    for (const star of state.stars) {
      ...
      const twinkleAmp = 0.04 + (1 - star.rank) * 0.24;
      ...
      const size = star.size * (1 + lift * rank * 0.28 + audio.bass * rank * 0.22 + audio.beat * rank * 0.12);
      ctx.fillStyle = rgba(star.r, star.g, star.b, alpha);
      ctx.beginPath();
      ctx.arc(sx, sy, size, 0, TWO_PI);
      ctx.fill();

      if (rank > 0.9 && settings.bloom !== false) {
        const halo = ctx.createRadialGradient(sx, sy, 0, sx, sy, size * (5 + rank * 8));
        halo.addColorStop(0, rgba(star.r, star.g, star.b, 0.12 * rank * (1 + audio.bass * 0.6)));
        halo.addColorStop(1, rgba(star.r, star.g, star.b, 0));
        ctx.fillStyle = halo;
        ctx.beginPath();
        ctx.arc(sx, sy, size * (5 + rank * 8), 0, TWO_PI);
        ctx.fill();
      }
    }
  }
...
function drawWarp(ctx, dt, now) {
  ...
  const speed = (state.reducedMotion ? 0.08 : 0.56) * (1 + audio.bass * 0.65 + audio.beat * 0.35);
  ctx.save();
  ctx.globalCompositeOperation = "screen";
