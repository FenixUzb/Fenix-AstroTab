...
function aiHubStarPalette(hdrEnabled = true) {
  ...
  const c = (r, g, b) => aiHubCanvasColor(r, g, b, 1, hdrEnabled);
  return [
    c(255, 255, 255), c(255, 255, 255), c(255, 255, 255), c(255, 255, 255),
    c(252, 252, 248), c(252, 252, 248), c(252, 252, 248), c(252, 252, 248),
    c(246, 250, 255), c(246, 250, 255), c(246, 250, 255), c(246, 250, 255),
    c(255, 252, 244), c(255, 252, 244), c(255, 252, 244), c(255, 252, 244),
    c(255, 232, 188), c(255, 232, 188),
    c(255, 192, 142),
    c(185, 213, 255)
  ];
}
...
function aiHubApplyStarGlowStyle(value) {
  ...
  document.documentElement.style.setProperty("--canvas-bloom-radius", Math.min(3.1, 1.55 + lift * 0.6).toFixed(2));
  document.documentElement.style.setProperty("--canvas-bloom-opacity", Math.min(0.78, 0.42 + lift * 0.16).toFixed(2));
  document.documentElement.style.setProperty("--canvas-bloom-overlay-opacity", Math.min(0.42, 0.22 + lift * 0.08).toFixed(2));
...
function aiHubSyncCanvasBloomFilter(enabled = null) {
  ...
  const radius = Math.min(4, Math.max(0, Number(styles.getPropertyValue("--canvas-bloom-radius")) || 1.95));
  const opacity = Math.min(1, Math.max(0, Number(styles.getPropertyValue("--canvas-bloom-opacity")) || 0.52));
  const blur = document.getElementById("aihub-star-bloom-blur");
  const lift = document.getElementById("aihub-star-bloom-lift");
  if (blur) blur.setAttribute("stdDeviation", bloomEnabled ? radius.toFixed(2) : "0");
  if (lift) {
...
function aiHubStarRankFromSize(size) {
  const n = Math.min(1, Math.max(0, (Number(size) - 0.45) / 0.9));
  return Math.pow(n, 2.35);
}
